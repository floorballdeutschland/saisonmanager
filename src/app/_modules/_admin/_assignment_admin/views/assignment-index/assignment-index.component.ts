...
// Länge der Kurzliste, hinterlegte Nummer eingerechnet. Weitere Namen oben verdrängen
// nur die alphabetische Liste, ohne die Auswahl weiter zu erleichtern. Der
// Deckel greift erst nach dem Schnitt mit den Kandidaten des Spieltags: Sonst
// bliebe die Kurzliste leer, sobald die fünf häufigsten Partner alle verplant
// sind, obwohl der sechste frei wäre.
...
export class AssignmentIndexComponent implements OnInit, OnDestroy {
  ...
  // Gespann-Historie je Schiri-PK, vollständig wie geliefert (der Deckel der
  // Kurzliste greift erst beim Abgleich mit der Tagesliste). Einmal je Person
  // geladen und über alle Zeilen geteilt: Der Ansetzer greift über viele Spiele
  // hinweg auf dieselben Namen zurück. Ein leerer Eintrag heißt „nichts zu
  // holen" – entweder wirklich ohne gemeinsame Einsätze oder strukturell
  // gesperrt (403/404), siehe Fehlerzweig in `_loadPartnerHistory`.
  ...
  // Sortiert nur um; welche Kandidaten überhaupt erscheinen, entscheidet
  // weiterhin `filteredReferees` (Vorfilter plus Suchtext) – Gespannpartner
  // sind davon nicht ausgenommen und verschwinden mit ihrer Lizenzstufe.
  ...
  onReferee2Focus(gameId: number): void {
    ...
    // schon mit, die Kurzliste soll auch dann oben stehen. Woher die
    // hinterlegte Nummer in diesem Fall kommt, steht in `_buildPartnerHints`.
    ...
    this._refereeService
      ...
      .subscribe({
        next: (list) => {
          ...
          state.partnerHints = this._buildPartnerHints(gameId, state);
          this._cdr.markForCheck();
        },
        error: () => {
          state.loadingReferees = false;
  ...
  onReferee1Input(gameId: number, value: string): void {
    ...
    state.showReferee1Dropdown = true;
    // Die Auswahl ist damit verworfen, also auch der Bezugspunkt der
    // Kurzliste. Ohne das Aufräumen blieben Hervorhebung, Badge und
    // Einsatzzahlen des vorherigen Schiri 1 im Dropdown von Schiri 2 stehen.
  ...
  selectReferee1(gameId: number, r: RefereeAssignmentAvailable): void {
    ...
    // Kurzliste für Schiri 2 vorbereiten: Die hinterlegte Nummer lässt sich
    // sofort auflösen, die Historie kommt nach.
    state.partnerHints = this._buildPartnerHints(gameId, state);
    ...
    this._refereeService
      ...
      .subscribe({
        next: (data) => {
          ...
          this._partnerHistory.set(refereeId, data.partners);
        ...
        error: (err: HttpErrorResponse) => {
          ...
          // vorgezogen (sie kommt aus der Kandidatenliste bzw. der Ansetzung).
          // Gemeldet wird deshalb nichts, der ErrorInterceptor lässt diesen
          // Endpunkt eigens durch.
          this._partnerHistoryLoading.delete(refereeId);
          // Nur Status und PK in die Konsole, nicht das Fehlerobjekt: Sentry
          // legt console-Aufrufe als Wegmarken ab und serialisiert deren
          // Argumente mehrere Ebenen tief, ein HttpErrorResponse trägt in
          // .error den geparsten Antwortkörper (#230).
          console.error(
            `Gespann-Historie nicht geladen (${err.status}): Schiri ${refereeId}`
          );
          // 403/404 sind strukturell (keine Zuständigkeit für die Person bzw.
          // zusammengeführtes Profil) und ändern sich in dieser Sitzung nicht:
          // leerer Eintrag, damit kein Fokuswechsel es erneut versucht. Alles
          // andere ist vorübergehend (5xx, kein Netz) und darf nicht dauerhaft
          // gecacht werden, sonst fehlt die Kurzliste dieser Person bis zum
          // Verlassen der Ansicht. Der nächste Fokuswechsel versucht es dann
          // neu – wie beim Laden der Kandidatenliste darüber.
  ...
  private _refreshPartnerHints(refereeId: number): void {
    this.rowStates.forEach((state, gameId) => {
  ...
  private _buildPartnerHints(
    gameId: number,
    state: RowState
  ): Map<number, PartnerHint> {
    ...
    // Schiri 1 steht nicht zwangsläufig in der Kandidatenliste: Der Server
    // nimmt tagesgleich schon angesetzte Personen heraus, also auch das
    // Gespann dieses Spiels selbst (Pokalspiele ausgenommen). Bei einer
    // bestehenden Ansetzung kommt die hinterlegte Nummer daher aus der
    // Ansetzung, sonst fiele die Kurzliste genau dort aus, wo sie am meisten
    // hilft.
    const referee1 =
      state.availableReferees.find((r) => r.id === referee1Id) ??
      this.rows.find((row) => row.game.id === gameId)?.assignment?.referee1;
    const declaredNumber = referee1?.partner_lizenznummer ?? null;
    ...
    history.forEach((p) => {
      ...
      if (rank >= MAX_PARTNER_SUGGESTIONS) return;
