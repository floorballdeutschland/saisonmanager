...
**Prod build uses prerender (SSG) — CI does not.** `build-deploy.sh` builds with `ng build --configuration production,prerender`, which pre-renders the public FD-league routes of the active season (route list generated by `scripts/generate-prerender-routes.mjs`, needs the API key). CI and `build-deploy-staging.sh` build **without** prerender (plain `production` config, like CI), so a prerender-only crash — e.g. a public league whose data trips a null-access during server-side rendering — passes CI and the staging deploy but **aborts the prod build** (no `dist/saisonmanager/browser/index.html` is produced; do NOT `scp` a failed build). Before a prod frontend deploy, run the prerender build locally to catch it:

```bash
export NVM_DIR="$HOME/.nvm" && . "$NVM_DIR/nvm.sh" && export PATH="$PATH:$(pwd)/node_modules/.bin"
ng build --configuration production,prerender    # must finish with "Prerendered N static routes" + emit index.html
```

Newly-hit prerender crashes usually appear when the active season gains its first public FD league to render (previously only `/` was prerendered), so re-run this check each release cycle.
...
Isolated stack alongside prod: `rails-api-staging` (branch `staging`), `postgres-staging`, and `mailpit` (catches **all** outgoing mail — nothing reaches real recipients). Serves an **anonymized clone** of the prod DB, behind Basic Auth + `noindex`. App login: any real (cloned) username + the shared staging password (set by the anonymize task; value in the private `saisonmanager-docker` repo).
...
- **Refresh staging DB from prod (anonymized):** `scripts/staging-db-refresh.sh` in `saisonmanager-docker` (also copies ActiveStorage files; resets **all** logins to the shared staging password)
...
- After a staging DB refresh, all logins reset to the shared staging password — re-set any custom demo passwords afterwards.

**Production server:** reachable via the `ssh saisonmanager` alias (host, user, and key live in the local `~/.ssh/config`; hardware-key auth)
...
- SSH needs the hardware key (touch + passphrase) — reliable only interactively (via the `!` prefix), not from non-interactive tool calls

**Archive server:** `archiv.saisonmanager.org` (SSH via the same hardware-key setup)
