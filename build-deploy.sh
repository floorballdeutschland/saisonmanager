...
if [ -f "src/environments/.google-client-id" ]; then
  ...
  if ! grep -q "GOOGLE_CLIENT_ID_PLACEHOLDER" src/environments/environment.prod.ts; then
    echo "Fehler: GOOGLE_CLIENT_ID_PLACEHOLDER nicht in src/environments/environment.prod.ts gefunden." >&2
    exit 1
  fi
  sed -i.sedbak "s|GOOGLE_CLIENT_ID_PLACEHOLDER|${GOOGLE_CLIENT_ID}|" src/environments/environment.prod.ts
  rm -f src/environments/environment.prod.ts.sedbak
